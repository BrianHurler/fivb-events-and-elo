...
raw_match_index <- raw_matches |>
  dplyr::transmute(
    match_id = as.character(no),
    raw_date = as.Date(local_date),
    raw_tournament_no = as.character(no_tournament)
  ) |>
  dplyr::distinct(match_id, .keep_all = TRUE)

new_match_index <- new_elo_matches |>
  dplyr::filter(as.Date(local_date) <= as_of_date) |>
  dplyr::transmute(
    match_id = as.character(no),
    new_date = as.Date(local_date),
    new_gender = as.character(gender),
    new_event_class = as.character(event_class),
    new_tournament = as.character(tournament_name_classified)
  ) |>
  dplyr::distinct(match_id, .keep_all = TRUE)

legacy_overlap <- legacy_match_index |>
  dplyr::left_join(
    raw_match_index |>
  ...
  dplyr::mutate(
    ...
    common_period = legacy_date >= as.Date("2008-01-01"),
...
legacy_overlap_summary <- dplyr::bind_rows(
  ...
  tibble::tibble(
    ...
    value = {
      ...
      c(
        nrow(x),
        sum(x$in_raw_archive),
        sum(x$in_new_elo),
        sum(x$in_raw_archive & !x$in_new_elo),
        sum(!x$in_raw_archive)
      )
    }
  )
...
match_overlap_by_year <- dplyr::full_join(
  legacy_match_index |>
    dplyr::mutate(year = as.integer(format(legacy_date, "%Y"))) |>
    dplyr::count(legacy_gender, year, name = "legacy_matches") |>
    dplyr::rename(gender = legacy_gender),
  new_match_index |>
    dplyr::mutate(year = as.integer(format(new_date, "%Y"))) |>
    dplyr::count(new_gender, year, name = "new_matches") |>
  ...
  dplyr::left_join(
    legacy_overlap |>
      ...
      dplyr::mutate(year = as.integer(format(legacy_date, "%Y"))) |>
      dplyr::count(legacy_gender, year, name = "shared_matches") |>
      dplyr::rename(gender = legacy_gender),
    by = c("gender", "year")
  ) |>
  dplyr::mutate(
...
new_shared_rows <- new_history |>
  ...
  dplyr::transmute(
    athlete_id,
    display_name,
    match_id = as.character(match_no),
    new_date = date,
...
shared_row_summary <- tibble::tibble(
  ...
  value = c(
    ...
    dplyr::n_distinct(shared_athlete_matches$match_id),
